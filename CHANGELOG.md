...
Integrate marker detection subscriber and refine callback logic for improved state transitions.

### Added
- feat(fsm): Added a new subscriber for the `/marker_detected` topic in `FSM/fsm_code.py`.

### Fixed
- fix(fsm): Refined the `aruco_callback` logic in `FSM/fsm_code.py` to ensure `msg.data` is checked alongside the "EXPLORE" state for marker detection.
