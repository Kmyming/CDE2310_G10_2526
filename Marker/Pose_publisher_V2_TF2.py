...
class ArucoTFBroadcaster(Node):
    def __init__(self):
        ...
        self.get_logger().info("ArUco dictionary: DICT_4X4_250")
    ...
    def camera_info_callback(self, msg: CameraInfo):
        if len(msg.k) == 9 and any(v != 0.0 for v in msg.k):
            ...
            if not self.logged_camera_info_once:
                self.get_logger().info("Using calibration from /camera/camera_info")
    ...
    def get_calibration(self):
        if self.have_camera_info and self.camera_matrix is not None:
            return self.camera_matrix, self.dist_coeffs, "/camera/camera_info"

        if self.use_fallback_calibration:
            if not self.logged_fallback_once:
                self.get_logger().warn("No valid /camera/camera_info found, using fallback calibration")
